...
import styles from './addRepeatingFailurePage.module.css'
...
export const AddRepeatingFailurePage: React.FC = () => {
  ...
  const getNotificationGroups = async () => {
    const dataRequest = `?company=${userContext.user?.companyName}&type=repeating`
  ...
  const updateNotification = async (
    ...
    form.repeatingFailure = dataState.name
  ...
  const removeFromGroup = async (
    ...
    if (notificationResponse.ok) {
      ...
      if (
        getRepeating().filter(
          (notification) =>
            notification.repeatingFailure === selectedGroup?.name
        ).length < 1
      ) {
  ...
  const getRepeating = () => {
    return notifications.filter(
      (notification) => notification.repeatingFailure !== (undefined || null)
    )
  }
  ...
  useEffect(() => {
    setInSelected(
      getRepeating().filter(
        (notification) => notification.repeatingFailure === selectedGroup?.name
      )
    )
  ...
  if (pageState === 1) {
    return !userContext || notificationGroupsLoad || notificationLoad ? (
      ...
      <div className={styles.container}>
        ...
        <div className={styles.title}>
          <Title title={'Add or edit repeating failure'} />
        </div>
        <div
    ...
    return (
      <div className={styles.container}>
        ...
        <Title title={'Add repeating failure'} />
        <div className={styles.data}>
          <InputField
            ...
            label="name"
            placeholder={
              dataState.name ? undefined : 'Name the repeating failure...'
            }
    ...
    return (
      <div className={styles.container}>
        ...
        <Title title={'Edit repeating error: '} dynamic={selectedGroup?.name} />
